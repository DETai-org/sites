...
- Тип `Publication` хранит весь контент страницы: `seoLead`, полный `abstract`, `keywords`, `citation`, `pdfUrl`, `externalLinks` (текст кнопок формируется как `Открыть в {label}`).
- Верх страницы строго: **H1** → строка метаданных (авторы · год · тип · журнал) → `seoLead` (если есть) → CTA-кнопки.
- CTA: всегда кнопка «Скачать PDF» (primary, `target="_blank"`); внешние источники показываются только если есть `externalLinks`.
