...
1. создать папку поста в `lib/blog/posts/<postId>/` и добавить Markdown-файлы переводов (`ru.md`, `en.md` и т.д.) с `descriptive.coverAlt` в frontmatter каждого языка;
2. добавить запись в [`blog.base.ts`](./blog.base.ts) согласно типам из [`types.ts`](./types.ts), указать `postId` (связка с frontmatter) и заполнить `contentFiles` (alt обложки берётся из frontmatter);
...
3. Заполните метаданные в `blog.base.ts`: `postId`, `publishedAt`, `rubric` (одна рубрика на пост), `category` (одна категория на пост), `keywords`, `keywordsRaw`, `coverImage`, `contentFiles`. Alt обложки хранится в frontmatter (`descriptive.coverAlt`) каждого языка.
