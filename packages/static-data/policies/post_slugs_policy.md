...
## 4) Где хранится
Данные slug постов хранятся **внутри сайта** (`detai-site` или `personal-site`)
**рядом с текстами постов** в `lib/blog/posts/...`.
...
Правило: для каждого поста **обязательны** `routeSlug` для **всех 5 языков**.
...
### 6.2. Язык `ru`
* предпочтительно транслитерация в латиницу
* допускается умеренная адаптация для читабельности

### 6.3. Язык `cn`
* если используется латинизация (pinyin) — только латиница и дефисы

### 6.4. Запрет на изменение `postId`
`postId` является стабильным и **не подлежит изменению**.

### 6.5. Изменение `routeSlug`
