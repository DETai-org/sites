---
type: post
administrative:
  id: "blizost-i-nezavisemost"
  authors:
    - "Anton Kolhonen"
  date_ymd: "2016-12-15"
  status: publish
  channels:
    - detai_site_blog
    - personal_site_blog
descriptive:
  routeSlug: "laheisyys-ja-itsenaisyys"
  title: "Läheisyys ja itsenäisyys"
  preview: "Läheisyys ja itsenäisyys on jännite yhteenkuuluvuuden kaipuun ja oman ainutlaatuisuuden säilyttämisen välillä. Tekstissä tuottava rakkaus kuvataan huolenpitona ja vastuuna toisen kasvusta sekä kunnioituksen tarpeena, ilman jota rakkaus muuttuu vallaksi. Mikä auttaa säilyttämään kunnioituksen, kun etsimme läheisyyttä? 💙"
  seoLead: "Läheisyys ja itsenäisyys ihmissuhteissa: rakkaus huolenpitona, vastuuna ja kunnioituksena toisen vapaudelle ja kasvulle."
  taxonomy:
    rubric_ids: ["rubric:det-notes"]
    category_ids: ["category:relationships-ideas"]
    keyword_ids: ["keyword:ambivalence"]
    keywords_raw: ["läheisyys ja itsenäisyys", "tuottava rakkaus", "kunnioitus ihmissuhteissa"]
structural:
  external_links: []
  document_links: []
---

# Läheisyys ja itsenäisyys

Ihmisen olemassaolon paradoksi on siinä, että ihminen etsii samaan aikaan sekä läheisyyttä että itsenäisyyttä, yhteenkuuluvuutta muiden kanssa ja oman ainutlaatuisuuden säilyttämistä... Tuottava rakkaus merkitsee huolenpitoa toisesta ihmisestä, vastuuntuntoa hänen elämästään — ei vain hänen fyysisen olemassaolonsa mielessä, vaan myös hänen mahdollisuuksiensa kasvusta ja kehityksestä. Tuottava rakkaus ei sovi yhteen passiivisuuden kanssa elämää kohtaan...

Näin ollen huolenpito ja vastuu ovat rakkauden erottamattomia ominaisuuksia, mutta ilman kunnioitusta sitä kohtaan, jota rakastaa, pyrkimys tuntea hänet yhä täydellisemmin ja syvemmin tekee rakkaudesta herruutta ja omistamista kuin esine. 💙

*** (Erich Fromm)***

> valokuva vuodelta 2011
