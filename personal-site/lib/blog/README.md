...
* Страницы постов (`/[lang]/blog/[slug]`) и листинги (`/[lang]/blog`) генерируются **на этапе сборки**.
* Используется `generateStaticParams()` для:

  * генерации всех языковых версий листинга;
  * генерации всех комбинаций `{ lang, slug }` для постов.
* Markdown-файлы читаются через `fs/promises` **только во время build**, а не на запросе.

### 🔹 Структура контента

* Каждый пост имеет **внутренний стабильный ID**.
